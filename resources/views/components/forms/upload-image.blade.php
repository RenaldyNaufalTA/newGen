...
<div x-data="fileUploadComponent()" class="col-span-full group mb-6">
    ...
    <label x-on:dragover.prevent="isDragging = true" x-on:dragleave.prevent="isDragging = false"
        ...
        <div class="text-center" x-show="!file">
        ...
        <template x-if="file">
            <div class="text-center -my-5">
                <img :src="imageUrl" alt="Uploaded Image" class="mx-auto h-48 w-48 object-cover rounded-lg">
                <p class="mt-2 text-sm font-medium text-gray-900" x-text="file.name"></p>
                <p class="text-xs leading-5 text-gray-600" x-text="(file.size / 1024).toFixed(2) + ' KB'"></p>
                <button type="button" @click="removeFile" class="btn-1 mt-2">Remove</button>
            </div>
        </template>
    </label>
